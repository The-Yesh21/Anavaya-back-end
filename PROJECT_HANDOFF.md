...
| **Case workflow** | Create New Case wizard (name or FIR or auto-ID `ANV-YYYY-NNNN`), multi-document evidence upload, analyze-all → **whole-case analysis** (features merged deterministically, Decision Tree run once on ALL evidence — `case_level`) + legacy highest-doc-wins aggregate; corroboration map, merge provenance, case-level PDF report | `scripts/case_manager.py`, `scripts/whole_case_analysis.py`, `scripts/whole_case_report.py` + Case tab |
...
| `POST /api/cases/{id}/analyze` | Analyze all unanalysed docs; compute whole-case analysis + refresh aggregate |
| `GET /api/cases/{id}/case-analysis` | Whole-case verdict: merged features, corroboration map, merge provenance, case-level constitutional opinion |
| `GET /api/cases/{id}/case-report.pdf` | Whole-case PDF report (all evidence, one verdict) |
...
| Date | Change |
|---|---|
| 2026-09-12 | **Fixed the destructive e2e cleanup that wiped the live case registry (incident):** `test_case_workflow_e2e.py`'s `restore_artifacts()` deleted **every** case JSON and the **entire** `case_documents/` tree instead of only its own test data — running the e2e against a live data dir destroyed case ANV-2026-0019 (registry JSON + 6 uploaded evidence files; uploads were never committed and `os.remove` skips the Recycle Bin). Fixed: cleanup now takes `test_case_ids` and removes only those cases' JSON + document subdirs. **ANV-2026-0019 was rebuilt** from `case_results.xlsx` (features, constitutional analysis, priorities) + the committed per-document decision reports/graphs/PDFs (commit 32c55ad); doc ids are freshly minted (`d_recovered_*`), original upload files are gone (downloads 404; a `LOST_IN_E2E_CLEANUP.txt` marker sits in the evidence dir — re-upload sources to restore them), text excerpts are empty so date/place corroboration is dormant until re-upload (party-based links still work: 4 links), and the Chakshu sessions/fact-checks for 0019 were lost. Whole-case analysis recomputed: Medium. Lesson: the e2e bootstraps a live-server-shaped environment — never point it at data you can't afford to lose (now it can't destroy any). |
...
| 2026-09-06 | **Fixed "Failed to delete document: Case 'ANV-…/documents/d_…' not found"** — evidence-document deletion was broken by route shadowing: `DELETE /api/cases/{case_file:path}` (delete case, app.py) matched multi-segment paths, so it swallowed `DELETE /api/cases/{case_id}/documents/{doc_id}` (FastAPI matches in registration order and delete-case is declared first) and returned 404 since a case id like `ANV-2026-0010/documents/d_c46dd48f` was never a real `Case_File` row. Removed the `:path` converter (frontend always DELETEs a single filename segment via `encodeURIComponent`, so it was never needed). Verified live on a temp server: document delete 200 (file removed from disk + registry), dashboard-style case delete by `Case_File` still 200. **Server restart required to pick up the fix.** |
| 2026-09-11 | **Courtroom deception analysis, case-linked trials, face-analysis accuracy rework:** (1) *Case context:* `case_manager.build_case_context()` builds a snapshot per case — main parties, per-document evidence summaries (from the stored pipeline analysis), deterministic evidence links (shared dates/places/parties via the fact-checker's extractors), Chakshu fact-check totals — persisted on the room at creation (`POST /api/court/rooms` now accepts `case_id`; title optional, defaults to the case title) and refreshable mid-trial (`POST /api/court/rooms/{id}/refresh-context`). The courtroom page shows it in a new **Case Context** panel (left column, with refresh button); the lobby's create-room form got a case-file selector and room cards show the linked case id; the transcript Markdown export includes the context. (2) *Session deception report:* new `scripts/courtroom_deception.py` — reuses fact_checker's claim/evidence extraction plus an LLM semantic layer to verdict every non-counsel statement as **lie** (contradicted by a collected document), **evasive** (non-answer), **missing_evidence** (asserted with no support on record — the "didn't mention the evidence ⇒ flagged" requirement), **consistent**, or **unverified**, with per-speaker credibility and a narrative disclaimer (flag ≠ judicial finding). Downloadable from the courtroom header/transcript bar and the ended-session card: `GET /api/court/rooms/{id}/session-report.pdf` (+ `.md`), computed fresh from the stored transcript. `statements_from_transcript()` keeps counsel dates with the following witness answer. (3) *Face analysis accuracy (calm faces no longer flagged):* cues now require the subject to be **speaking** (self mic / remote feed RMS gate via AnalyserNode; no meter ⇒ cues stay off — safe direction), thresholds raised from 2.0–2.2σ to 2.8–3.0σ, persistence gate 6→45 frames (~0.75 s), the gauge is an EMA (α=0.06) with a quiet-decay and a 0-floor, and transcript logging needs ≥2 concurrent cues + 3 s speech history + index ≥25 with a 25 s cooldown. On stop, a **whole-session aggregate** (duration, speaking/calm seconds, mean/peak index, per-cue seconds) is sent to `POST /api/court/rooms/{id}/face-summary` (replaces same-source), logged as one summary behavior entry, rendered in the transcript exports, and included in the session report. (4) *Context-aware ASR correction:* `correct_transcript_text(text, context_hint)` — rooms linked to a case pass title/parties/evidence names so whisper mishearings of case names are restored (typed statements route the same hint through `/api/court/correct-transcript` with `room_id`). Verified: py_compile + node --check clean, deception engine unit-test (lie/evasive/consistent/unverified all correct), courtroom e2e **14/14** and case-workflow e2e pass (e2e scripts now honor `ANAVAYA_TEST_BASE`), face-summary/session-report/refresh-context endpoints exercised live, test rooms cleaned up. Note: the courtroom e2e is slow (~4 min; 6 s WS drain timeouts) — not a hang. |
| 2026-09-06 | **Case deletion + auto-delete on empty:** Added `delete_case()` to `case_manager.py` (removes JSON registry + evidence directory + sessions), a unified `DELETE /api/cases/{case_file}` endpoint in `app.py` that handles both case-id deletes (full registry cleanup + Excel row removal) and document-filename deletes (Excel-only), and auto-deletion of a case when its last document is removed via `delete_document()`. Frontend gets a red **Delete Case** button in the case workspace (`lobby-delete-btn`) and a trash button on each registry card (`registry-case-delete`). Verified live via ngrok tunnel: case deletion 200, evidence dir + JSON removed, registry refreshed, auto-delete on last-doc removal confirmed. **Server restart required.** |
| 2026-09-11 | **Whole-case analysis — one verdict over ALL evidence (replaces per-document-only analysis):** "Analyze All Documents" used to run the pipeline per document and the case "aggregate" was just highest-doc-wins; the officer got N separate analyses instead of an analysis of the case. New `scripts/whole_case_analysis.py`: (1) **deterministic merge** of per-document features into ONE case-level feature set (worst severity / highest vulnerability / highest influence wins, majority `case_category`+`crime_type` with safety-first tie-breaks, parties union, whole-case narrative naming every document — `merge_info` records which documents drove each decision); (2) **corroboration map** — which documents share dates/places/parties (reuses the fact-checker's deterministic extractors) and which stand alone unsupported; (3) the **Decision Tree runs ONCE on the merged features** — the case's own verdict — with the rule-based constitutional analysis at case level. Founding invariant preserved: no LLM anywhere in the merge/priority path. New `scripts/whole_case_report.py` renders `reports/{case_id}_whole_case_report.pdf` (merge-rules + corroboration + constitutional sections). `Case.case_level` persisted in the registry JSON; `refresh_aggregate()` now computes it (legacy `aggregate_priority` kept for dashboard/courtroom compatibility). New endpoints `GET /api/cases/{id}/case-analysis` and `GET /api/cases/{id}/case-report.pdf`; old cases backfill `case_level` lazily on first request. UI: **Whole-Case Analysis** panel in the case workspace (verdict badge, whole-case narrative, classification chips, corroboration list, auditable merge provenance, case-level opinion, PDF download); Excel gains one `[WHOLE CASE] <title>` row per case; dossier export + courtroom Case Context show the whole-case verdict. Verified: smoke test on the real 6-doc case ANV-2026-0019 (merge provenance, 5 corroboration links, tree Medium, PDF generated); `test_case_workflow_e2e.py` passes; live endpoint test with 2 docs (per-doc Medium+High → case-level High, PDF 200/368 KB, Excel row, dossier section). **Server restart required.** |
| 2026-09-11 | **Remote stack restarted after reboot (no code changes):** machine had rebooted since the 2026-09-04 demo — all services (8000/8083/8084/4040) were down. Ran `deploy/start-anavaya.ps1` from the repo root; backend + nginx + landing + ngrok + Ollama all came up (first boot takes >4 min — vite landing startup is the slow part; the launcher is idempotent, so re-running is safe and `Wait-Port` just looks slow). Verified end-to-end: backend :8000 → 200, nginx :8083 `/` and `/landing/` → 200, ngrok tunnel open on the stable domain `https://overreach-headrest-gosling.ngrok-free.dev` (public `/` and `/landing/` both 200), `/api/gpu-status` reports `llm_mode=enabled` · RTX 2050 · qwen2.5:3b. This fresh boot also picked up all the 2026-09-06 code changes (deception report, case-linked trials, face-analysis rework, case deletion) that were waiting on a restart. Standing reminder: `C:\nginx\Anavaya-Remote.exe` is still the pre-GPU packaging — re-sync/re-package when convenient. |
